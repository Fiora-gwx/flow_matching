...
def _plot_main_curve(rows: Sequence[Dict[str, object]], output_dir: Path) -> None:
    ...
    for label, group_rows in sorted(grouped.items()):
        ...
        y = [row["value"] for row in ordered]
...
def _cross_path_table(rows: Sequence[Dict[str, object]]) -> List[Dict[str, object]]:
    ...
    for row in rows:
        ...
        if current is None or float(row["value"]) < float(current):
        ...
        output.append(
            {
                ...
                "solver": entry["solver"],
                "nfe": entry["nfe"],
                "linear_uniform": entry.get("linear:uniform"),
                "linear_ft": entry.get("linear:ft_linear_beta"),
                "trig_vp_uniform": entry.get("trig_vp:uniform"),
                "trig_vp_ft": entry.get("trig_vp:ft_vp_beta"),
            }
        )
    return output


def visualize_results(csv_path: Path, output_dir: Path, artifact_group: str = "") -> None:
    ...
    main_table = baseline_vs_best_beta(linear_rows)
...
if __name__ == "__main__":
    ...
    visualize_results(args.csv, args.out, artifact_group=args.artifact_group)
